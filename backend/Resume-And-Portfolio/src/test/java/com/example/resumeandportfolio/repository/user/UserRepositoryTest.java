...
public class UserRepositoryTest {
    ...
    @Test
    @DisplayName("이메일로 활성 사용자 찾기 - 성공")
    void findByEmailAndDeletedAtIsNullSuccess() {
        // Given: 테스트 데이터 삽입
        User user = User.builder()
        ...
        // When: 활성 사용자 조회
        Optional<User> result = userRepository.findByEmailAndDeletedAtIsNull("active@example.com");

        // Then: 결과 검증
        assertThat(result).isPresent();
        assertThat(result.get().getEmail()).isEqualTo("active@example.com");
        assertThat(result.get().getNickname()).isEqualTo("ActiveUser");
    }

    @Test
    @DisplayName("이메일로 활성 사용자 찾기 - 실패 (삭제된 사용자)")
    void findByEmailAndDeletedAtIsNullFailure() {
        // Given: 삭제된 사용자 삽입
        User user = User.builder()
